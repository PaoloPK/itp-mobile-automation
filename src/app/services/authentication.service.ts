...
export class AuthenticationService {
  ...
  login(id: string, password: string, plant_code: string): Observable<AuthResponse> {
    const url: string = this.host + 'itproxy/public/login';
    const user = { 'username': id, 'password': password, 'plant_code': plant_code };
    return this.http.post(url, user).pipe(
      tap((res: AuthResponse) => {
        ...
        const url = this.host + 'itproxy/public/api/impianto/' + plant_code;
        console.log(url);
        const httpOptions = {
  ...
  loginWithCode(code: string) {
    const route: string = 'http://' + this.host + 'logincode/' + code;
  ...
  registerOnesignal() {
    ...
    this.oneSignal.handleNotificationReceived().subscribe((data) => {
      ...
      const type = data.payload['additionalData']['type'];
      const machine = data.payload['additionalData']['machine'];
      const message = data.payload['additionalData']['message'];
